...
import { makeFindUserRequest } from 'test/factories/requests/user-factories/find-user-request'
...
describe('find user controller', () => {
  ...
  test('GET /users/:role/:id', async () => {
    ...
    const findResp = await makeFindUserRequest(app, {
      role: 'courier',
      token,
      userId: courier.id,
    })

    expect(findResp.statusCode).toEqual(200)
    expect(findResp.body.courier).toEqual(
