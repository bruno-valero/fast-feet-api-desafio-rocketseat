...
import { makeFetchUsersRequest } from 'test/factories/requests/user-factories/fetch-users-request'
...
describe('fetch users controller', () => {
  ...
  test('GET /users/:role', async () => {
    ...
    const fetchResp = await makeFetchUsersRequest(app, {
      role: 'courier',
      token,
    })

    console.log('fetchResp.body', fetchResp.body)
    expect(fetchResp.statusCode).toEqual(200)
    expect(fetchResp.body.couriers).toHaveLength(1)
    expect(fetchResp.body.couriers[0]).toEqual(
