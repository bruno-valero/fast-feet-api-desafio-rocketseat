...
import { makeDeleteUserRequest } from 'test/factories/requests/user-factories/delete-user-request'
...
describe('delete user controller', () => {
  ...
  test('DELETE /users/:role/:id', async () => {
    ...
    const deleteResp = await makeDeleteUserRequest(app, {
      token,
      role: 'courier',
      userId: courier.id,
    })

    const couriers = await prisma.prismaUser.findMany({
      where: {
        role: 'courier',
      },
    })
