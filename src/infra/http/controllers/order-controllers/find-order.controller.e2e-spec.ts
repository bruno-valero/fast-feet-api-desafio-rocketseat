...
import { makeFindOrdersRequest } from '../../../../../test/factories/requests/order-request-factories/make-find-order-request'
...
describe('find order controller', () => {
  ...
  test('GET /orders/:id/find', async () => {
    ...
    const findOrdersResp = await makeFindOrdersRequest(app, {
      orderId: orders[0].id,
      token,
    })
    ...
    expect(findOrdersResp.statusCode).toEqual(200)
    expect(findOrdersResp.body.order.courier).toEqual(
      expect.objectContaining({ id: courier.id }),
    )
  })
})
