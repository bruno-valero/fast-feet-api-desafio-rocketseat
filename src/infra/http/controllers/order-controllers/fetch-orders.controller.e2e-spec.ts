...
describe('fetch orders controller', () => {
  ...
  test('GET /orders/:role(courier)?userId=(uuid)', async () => {
    // admin already crated on first test
    const authAdmResp = await makeAuthenticateRequest(app, {
      body: {
        cpf: '20635940078',
        password: '123',
  ...
  test('GET /orders/:role(recipient)?userId=(uuid)', async () => {
    // admin already crated on first test
    const authAdmResp = await makeAuthenticateRequest(app, {
      body: {
        cpf: '20635940078',
